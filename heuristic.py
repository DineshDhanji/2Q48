...
class Solver:
    ...
    def score_snake(self, grid, base_value=0.25):
        ...
        score = 0
        for i in range(2):
            gridArray_horizontal = np.hstack(
                tuple(grid[j] if i % 2 == 0 else grid[j][::-1] for j in range(size))
            )
            score = max(score, np.sum(rewardArray * gridArray_horizontal))
            score = max(score, np.sum(rewardArray[::-1] * gridArray_horizontal))
            gridArray_vertical = np.hstack(
                tuple(grid[j][::-1] if i % 2 == 0 else grid[j] for j in range(size))
            )
